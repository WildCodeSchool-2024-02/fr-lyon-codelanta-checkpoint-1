...
function findAdults(persons) {
  const adult = persons.filter(persons => persons.age >= 18);
  const femaleAdult = adult.filter(persons => persons.sex ==="female");
  const maleAdult = adult.filter(persons => persons.sex ==="male");
